...
import type { CreateItemInput, DashboardResponse, ExpirationItemStatus, ItemResponse, RenewItemInput } from "./types.js";
...
export function renewItem(itemId: string, input: RenewItemInput, expectedVersion: number, idempotencyKey: string): Promise<ItemResponse> {
  return apiClient.post<ItemResponse>(`/items/${encodeURIComponent(itemId)}/renew`, input, { idempotencyKey, expectedVersion });
}
