...
import type { PolicyResponse, PutPolicyInput } from "../api/types.js";
...
export function useSaveReminderPolicy(itemId: string) {
  ...
  return useIdempotentMutation<PolicyResponse, SaveVariables>({
    ...
    onSuccess: () => {
      if (organizationId) void queryClient.invalidateQueries({ queryKey: queryKeys.items.reminderPolicy(organizationId, itemId) });
    },
  });
}
