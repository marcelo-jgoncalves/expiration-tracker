 ...
 * guaranteed-failing request). PERF-10: REFERENCE - a tenant-wide setting an OWNER edits rarely.
 * `useUpdateDocumentRequestDeliveryPreference` invalidates this key precisely on save, so the
 * cache window never shows a stale value right after the OWNER changes it. */
