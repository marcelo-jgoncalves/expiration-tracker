...
- **[regra obrigatória]** (Marcelo, 2026-08-31) Todo retorno sobre trabalho concluído deve incluir, de forma concisa, o que ainda falta fazer — nunca só "o que terminou". Aplica-se a qualquer resumo de status, não só ao fim de uma tarefa grande; a lista de pendências deve refletir o estado real da fila (`NEXT_SESSION_PROMPT.md`), não uma lista solta reconstruída de memória.
- **[regra obrigatória]** (Marcelo, 2026-09-10) Quando um subagente/fork delegado (ex. `Agent` com `subagent_type: "fork"` para uma implementação longa) ficar 15 minutos rodando sem enviar nenhuma atualização, pedir explicitamente um status (onde está no fluxo, algum bloqueio/achado inesperado, estimativa do que falta) em vez de só esperar passivamente a notificação de conclusão — isso já pegou cedo um caso real de corte por limite técnico de turnos (200 turnos) que, sem o status-check, só teria aparecido na notificação final como resultado parcial sem aviso prévio.
- **[regra obrigatória]** (Marcelo, 2026-09-17) Em operações longas com progresso consultável, como pipelines CI/CD, deploys e injeções de carga, não fazer polling contínuo. Estimar quando a operação deve estar perto do fim e agendar uma única avaliação para essa janela. Se ainda estiver em andamento, usar o estado e o ritmo observados para definir a próxima avaliação pontual. Em testes de carga, incluir avaliações de AWS nos marcos úteis combinados (por exemplo, aproximadamente na metade da injeção) sem acompanhar cada incremento. O objetivo é preservar tokens sem reduzir a qualidade da validação; antecipar a avaliação somente diante de erro, alerta ou risco de perder uma janela operacional.
