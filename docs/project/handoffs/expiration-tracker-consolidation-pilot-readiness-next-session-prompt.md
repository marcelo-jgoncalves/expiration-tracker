...
### Concluído nesta sessão (2026-08-28)

- [x] Corrigir drift de documentação do Pilot Readiness Program (Wave 6 status, W2-02/W3-01 status, `docs/frontend/README.md` data, marcador `PENDING` em W3-09) — commit `88e5eda` (sessão anterior).
- [x] **W2-01 — auto-confirm escreve `dueDate` automaticamente**: implementação encontrada pronta e não commitada (background da sessão anterior), verificada (typecheck/lint/913 testes de backend) e commitada — `e9f2439`. D-058 em `decisions-log.md`.
- [x] **W5-01 — GTR-01 (identidade do requester exposta ao guest)**: as 3 perguntas de escopo de produto pendentes (o quê capturar/obrigatoriedade/fallback) foram decididas por recomendação nesta sessão (nome do tenant, opcional, fallback genérico "Solicitante não identificado" — nunca inferido). Implementado de ponta a ponta: `UserProfile.requesterDisplayName`, `GET/PUT /profile` (novo módulo HTTP+Lambda+rotas Terraform+BFF allowlist), `GuestSubmissionService.getRequestInfo()`, 2 templates de e-mail guest-facing — commit `7dacbac`. D-060 em `decisions-log.md`. 928 testes de backend (era 913), `terraform test` real verde (api-gateway + raiz).
- [x] **Corrigir referência local `main` desatualizada**: era staleness real do ref local (`git fetch origin main:main`), não um bug de leitura — corrigido, `main`/`origin/main` sincronizados.
- [x] **Registrar formalmente em `decisions-log.md`**: D-058 (W2-01), D-059 (W3-06 — decisão de produto registrada, mecanismo ainda pendente), D-060 (W5-01, atualizado para `IMPLEMENTADO` após o trabalho desta sessão).
- [x] `docs/engineering/pilot-readiness-program.md` Wave 5 atualizada para `DONE`; `pilot-readiness-assessment.md` ganhou um addendum datado (síntese original preservada); `NEXT_SESSION_PROMPT.md` reconciliado.

### Ainda pendente (próxima sessão)

- [ ] **W3-06 — purge real de `USER_DOCUMENT`**: decisão de produto já tomada (priorizar agora, D-059) — o que falta é o **desenho do mecanismo**, que É uma decisão de arquitetura Type 1 (nível 5-6) e precisa do protocolo Claude↔Codex completo (`AGENTS.md` §4: mínimo 3 rodadas, nota cega, ambos ≥9.0) antes de implementar. Não iniciado nesta sessão — escopo grande demais para encaixar depois de W2-01/W5-01 sem apressar o protocolo. Padrão de referência: `EXTRACTION_TRANSIENT` (worker explícito de purge + lifecycle S3 como rede de segurança, `src/modules/extraction/domain/retention.ts` + `infra/main.tf:1625-1642`). W3-07 (cascata de deleção real para DSR) usa o mesmo mecanismo — escopar os dois juntos é o caminho eficiente, mas não expandir further sem necessidade.
