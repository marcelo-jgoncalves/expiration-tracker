...
| InitialInviteRateLimit *(M10 cluster 4, D-049)* | contador de janela fixa — 3 itens por tentativa (hora/dia por tenant, 24h por tenant+`recipientEmailHash`), mesmo mecanismo de `GuestTokenRateLimit`, duplicado deliberadamente para não acoplar `subject` a `identity`'s `TenantQuotaService` | `TENANT#t#SETTINGS` / `RATE` ou `RATE_DAILY`; `TENANT#t#SETTINGS#RECIPIENT#<hash>` / `RATE` |
| ImportJob *(M11, D-042, roadmap-evolution/09-domain-model-csv-import.md — agregado próprio, coleção sob a própria partição, nunca sob subject)* | jobId, targetEntityType (`TrackedSubject` apenas em v1), status (UPLOADED/PARSING/PREVIEW_READY/COMMITTING/COMMITTED/FAILED/EXPIRED), checksumSha256, mappingVersion, totalRows/acceptedRows/rejectedRows/duplicateRows, planObjectKey/planSha256 (referência ao plano em S3, nunca linha-a-linha em DynamoDB), lastCommittedRowNumber (cursor de retomada segura do commit), failureReason?, expiresAt (TTL lógico de 7 dias), version | `TENANT#t#IMPORTJOB#j` / `META` |
| ImportDedupRecord *(M11, D-042)* | externalId (real ou sintético `job:<jobId>:row:<rowNumber>` para linha sem externalId), subjectId criado — serve dedupe entre imports E idempotência de retry do commit ao mesmo tempo (claim-first, antes de chamar `SubjectService.createSubject()`) | `TENANT#t#IMPORTDEDUP#<externalId>` / `META` |
