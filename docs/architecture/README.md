...
| `roadmap-evolution/17-multi-user-b2b-revised-strategy.md` | decisão reconciliada via protocolo AGENTS.md §4 (qualidade técnica) — timing reservado ao Marcelo | Revisão estrutural de Multi-User B2B — `Organization` como tenant boundary permanente, `User` global, `Membership` N:N, RBAC por permissions. Protocolo Claude↔Codex completo (§125 do próprio documento): 3 rodadas reais, Claude 9,2/Codex 9,2, ambos ≥9,0. **Achado central da Rodada 1**: o documento recomendava "fazer agora" sem reconciliar `roadmap-evolution/05` (já `APPROVED`, decide o oposto — RBAC só no gatilho comercial real) — corrigido explicitando que este documento propõe supersessão só do *timing*, decisão reservada ao Marcelo, nunca ao protocolo. Outros achados fechados: mecanismo transacional de last-OWNER (`ownerCount`), unicidade de Membership/Invitation pendente, tabela mantém/emenda/refaz do impacto no W3-07, supersessão explícita de D-060 (GTR-01) |
