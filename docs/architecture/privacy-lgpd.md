...
## 7. Nota de status de implementação (corrigido 2026-09-25, auditoria de arquitetura de dados/item 30 — parágrafo anterior de 2026-08-20 ficou stale desde D-061/D-127 sem revisão)
Este documento é design aprovado (Design Maturity `APPROVED`), não um relatório do que já existe em código. Estado real confirmado nesta auditoria (ver `data-model.md` §1 para o detalhe físico):
- **Implementado**: minimização de telemetria (`src/shared/observability/redactor.ts` + `schemas/sensitive-fields.json`); trilha de auditoria transacional e redigida para `ExpirationItem`; **purga física real de 8 das 9 classes/legs de `retentionClass`** (§4) — `USER_DOCUMENT` (D-061), `CORE_USER_DATA`/`DELIVERY_RECORD`/`SECURITY_AUDIT`/`QUOTA_TELEMETRY`/`TRANSIENT`/`ACCOUNT_ACTIVE`(`Invitation`+`Membership`) via as 7 prioridades de D-127 (workers em `src/workers/{document,core-user-data,delivery-record,invitation,membership,quota-telemetry,security-audit,transient}-purge/`, agendados via `MaintenanceDueIndex`/GSI8, D-179-190).
- **Não implementado ainda**: `DataSubjectRequest` e qualquer endpoint de confirmação/acesso/correção/exportação/oposição/exclusão (§3, ainda escopo de produto real, não iniciado); `LEGAL_EVIDENCE` (Prioridade 7 de D-127, bloqueada por decisão jurídica — nenhum documento é classificado nessa classe hoje, correto adiar); purga de `NotificationPreferences` órfã após remoção de `Membership` (achado de D-169: falta estado terminal próprio, decisão de design nova necessária, não esquecimento); allowlist/SCP de região e inventário versionado de subprocessadores (§5).
- Classificação: DSR endpoints reais seguem escopo maior (feature de produto); região/subprocessadores é impedimento externo (decisão de negócio + parecer jurídico pendentes); `NotificationPreferences` é a única lacuna de purga genuinamente aberta e tecnicamente pronta para ser escopada — candidata a entrar na fila do protocolo Claude↔Codex quando priorizada. Nenhum item aqui é drift silencioso — todos rastreados nesta seção e em `decisions-log.md`.
