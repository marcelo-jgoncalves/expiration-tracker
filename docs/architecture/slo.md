...
Base: `docs/architecture/history/slo/slo-claude-round1.md`, `docs/architecture/history/slo/slo-codex-round1.md`, `docs/architecture/capacity-model.md`, `docs/architecture/architecture-fase3-consolidada.md`, `docs/architecture/data-model.md`. Seção 39 do prompt mestre.
...
- **Rodada 2** — concluída. Codex reagiu aos 3 pontos abertos do Claude (ver "Resultado da avaliação" acima): SLO de visibilidade de rejeição formalizado (§4), dashboard e janela do Stage 5 registrados como não-bloqueantes/estimativa não validada em vez de deixados implícitos. Status final: **APPROVED**. A lista "Pontos abertos" ao final deste documento não significa "rodada pendente" — são lacunas reconhecidas e conscientemente não-bloqueantes (achado corrigido em `full-audit-round1-operacoes-*`, 2026-08-20: este parágrafo dizia "pendente" enquanto o resultado acima e o restante do documento já registravam a Rodada 2 como resolvida — drift de dois estados divergentes no mesmo arquivo).
...
## Pontos abertos (lacunas reconhecidas, não-bloqueantes para o APPROVED acima — reagidas na Rodada 2, não fechadas por completo)
1. Sem SLO de latência específico para o dashboard (queries GSI1/GSI2 potencialmente mais pesadas que outros endpoints da API) — só a meta genérica de API se aplica hoje. Reação do Codex (Rodada 2): não-bloqueante antes do Stage 3, revisar quando o dashboard tiver tráfego real.
2. O SLO de UX de rejeição foi formalizado na Rodada 2 (seção 4, "Visibilidade de rejeição") — item fechado, mantido aqui apenas como registro histórico de que a Rodada 1 o havia identificado como faltante.
3. Janela de reconciliação do Stage 5 (≤12h) é estimativa não validada por nenhum teste real de throughput de scan em lote sobre ~8M itens — registrado como tal, não fingindo certeza. Critério de saída formal antes de operar em Stage 5 real: `disaster-recovery.md` §8 (3 execuções consecutivas sob carga).

## Error budget — política provisória Stage 0-1 (achado corrigido em `full-audit-round1-operacoes-*`, 2026-08-20)
Sem tráfego de produção real ainda, não há dado de consumo de error budget a medir — o critério de "error budget condiciona release" (`joint-review-criteria.md`, eixo Operações, item 1) não pode ser operacionalizado com dado real hoje; isso é impedimento externo genuíno, não omissão. Política provisória até existir volume mensurável (Stage 2+, `evolution.md`): (a) qualquer alarme de erro de função do pipeline assíncrono (`infra/lib/reminder-observability.ts` / `infra-terraform/modules/reminder-observability/`) disparando em produção pausa deploys de mudança não-crítica até investigação, tratado como "budget zerado" de fato mesmo sem cálculo formal de burn rate; (b) a partir do primeiro mês com tráfego real mensurável, recalcular os SLOs da seção 2 como budget real (100% − meta) e revisar esta política para burn-rate multi-janela (padrão Google SRE, 1h/6h) em vez do gate binário acima.
