...
**Correção factual (2026-09-20, achado ao ler `frontend/src/styles/tokens.css` real antes de implementar a Fase de portação — nunca presumir sem ler o código)**: a versão original desta ADR afirmava que a cor de acento em produção ainda era azul-indigo. Não é. **D-135 (2026-08-31) já trocou `--color-accent-*` para violeta `#7C3AED`** em `frontend/src/styles/tokens.css` — decisão direta de Marcelo, mesma exceção de `ai-governance.md` §2, comparando o preview publicado contra o mesmo mockup `Expiration Tracker - Prototipo Standalone (1).html` que origina este workstream. Contraste já verificado então (accent-600 5,70:1, accent-700 como link 7,10:1). Só a camada de primitivos mudou; tipografia, radius e sombra ficaram **explicitamente fora do escopo de D-135** — são exatamente os três pontos que esta ADR resolve agora.

`docs/frontend/visual-language-and-design-system.md` (16 rodadas, `VL-G1..VL-G17`, Claude 9,2/Codex 9,04, 2026-08-26) e `docs/frontend/design-system.md` (5 rodadas, Claude 9,2/Codex 9,5, D-130, 2026-08-31) formalizaram a direção visual original — acento azul-indigo (revertido por D-135, ver acima), stack tipográfica de sistema sem webfont, **nenhum sistema de ícones** (só marcadores de forma CSS e glifos de texto), radius restrito (4/6/8px). O primeiro documento permanece, até hoje, marcado `PROVISIONAL PENDING USER VALIDATION`, e D-135 já registrou como pendência explícita que a prosa de ambos os documentos ficou desatualizada pela troca de cor — pendência que esta ADR também fecha.

Marcelo explorou uma direção v2 em sessão separada (Claude Design/Artifacts, fora deste repositório), produzindo um design system completo (tokens, 21 componentes, camada de refinamento, config de lint de aderência) com o **mesmo acento violeta `#7c3aed`** (já vigente desde D-135) **mais** tipografia Plus Jakarta Sans (Google Fonts), radius maior (8/12/18px) e ícones reais (Lucide, SVG embutido) — os três pontos genuinamente novos. Entregue como `Untitled.zip` em `prototype/`, auditado nesta sessão (contraste recalculado, `forced-colors`, `WCAG 2.2` SC 2.5.8/2.4.11, achados reais corrigidos) contra os critérios já `APPROVED` de `docs/frontend/frontend-engineering-quality-standard.md` (Eixo 3 — Acessibilidade 12%, Eixo 11 — Design System 3%) e `docs/frontend/interface-quality-standard.md`.

Marcelo decidiu diretamente, 2026-09-20: **este v2 (já corrigido) é a base do design system oficial**, substituindo a direção original nos pontos onde divergem — confirmando a cor que D-135 já tinha decidido, e decidindo tipografia/radius/ícones agora pela primeira vez.

## Options Considered

1. **v2 — acento violeta (já vigente desde D-135), Plus Jakarta Sans, radius 8/12/18px, ícones Lucide** (escolhida) — decisão direta do responsável final por identidade visual/arquitetura (`AGENTS.md` §1). Já auditado contra os eixos de acessibilidade/design system já aprovados neste projeto, com 4 achados reais corrigidos antes desta ADR (ver §"Correções aplicadas").
2. **Original — stack de sistema sem webfont, radius 4/6/8px, sem ícones** (`visual-language-and-design-system.md`/`design-system.md`, hoje ainda vigente no `frontend/src/components/ui/**` real para ESSES três atributos — a cor já é violeta desde D-135) — rejeitada por decisão direta de Marcelo, não por mérito técnico comparativo. Nunca chegou a passar por User Validation real (ficou `PROVISIONAL` desde 26/08).
