...
**Status:** proposta independente para debate Claude ↔ Codex  
**Estágio:** Design Maturity APPROVED → Implementation Blueprint  
...
| `ExtractionTaskHandler` | Step Functions | Tipo, OCR, parsing, Bedrock, validação e comparação |
...
A primeira versão pode empacotar tarefas pequenas de extração em um único `ExtractionTaskHandler`, desde que cada estado da Step Functions invoque uma operação nomeada e independente. Tarefas com permissões distintas, especialmente Bedrock e parsing de PDF, devem tornar-se Lambdas separadas antes de produção pública.
...
- Preferência: sessão BFF com cookie `HttpOnly`, `Secure`, `SameSite=Lax/Strict`, ou, se o browser usar Cognito diretamente, refresh token restrito a cookie seguro controlado por endpoint de sessão.
...
- CSP inicial: `default-src 'self'`; scripts por hash/nonce; sem `unsafe-inline`; `connect-src` limitado ao domínio da API e endpoints Cognito necessários; `frame-ancestors 'none'`; `object-src 'none'`; `base-uri 'self'`.
...
- lookup Cognito: índice dedicado ou item de mapeamento único, sem usar e-mail;
...
Consumer do Reminder cancela ocorrências antigas por condição de versão e materializa as novas. Evento repetido produz o mesmo resultado.
...
```json
{
  ...
  "status": "SCHEDULED",
  "version": 1,
  "GSI3PK": "TENANT#t_01#DUE#202609101200#02",
  "GSI3SK": "2026-09-10T12:00:00.000Z#occ_01"
}
```
...
```text
shard = stableHash(tenantId + "|" + occurrenceId) mod 4
```

A quantidade de shards é configuração versionada. Dobrar shards exige período de leitura sobre conjunto antigo+novo até toda a janela ser rematerializada.

### 9.3 Producer por minuto

Para minuto `M`, `ReminderProducer`:

1. obtém configuração de shards;
2. consulta GSI3 para os quatro shards de `M` e, por tolerância, minuto anterior ainda aberto;
3. envia uma mensagem por ocorrência;
4. não muda estado para `TRIGGERED`;
5. em falha parcial de batch, reenvia somente entradas falhas;
6. emite `scheduler_lag_seconds` e quantidade por shard.
...
- ocorrência deve estar `SCHEDULED`;
...
- cria outbox `NotificationIntentCreated`.

Se item/política estiver stale:

- cancela a ocorrência condicionalmente;
- se ainda existir intenção legítima para a regra atual, agenda reparo;
...
- quatro shards são consultados e monitorados;
- runbook de expansão de shards testado;
...
1. verifica kill switch do canal;
...
1. `LoadMetadata`
2. `DetectDocumentType`
3. `RunTextract`
4. `RunDeterministicParser`
5. `NeedsBedrock?`
6. `CheckAiKillSwitch`
7. `RunBedrock`
8. `ValidateSchema`
9. `CompareExtractors`
10. `PersistExtractedFields`
11. `MarkPendingConfirmation`
12. `CompleteRun`
...
Eventos são append-only. A role de aplicação possui `PutItem` condicional, não `UpdateItem`/`DeleteItem`, para itens de auditoria.
...
- AI/OCR/WhatsApp: desligado;
- e-mail essencial: política explícita, preferencialmente fail-closed até a configuração ser conhecida.
...
WhatsApp é submilestone posterior e não bloqueia e-mail.
...
## 23. Decisões de implementação que exigem ADR antes do código

Este blueprint não deve escolher silenciosamente:

1. BFF de sessão versus Cognito direto no browser; ambos devem cumprir o modelo de armazenamento e revogação.
2. Representação exata do lookup `cognitoSub → userId`.
3. Provider inicial de e-mail e seu modelo idempotente.
4. Estratégia concreta de backup S3 que satisfaça RPO ≤24h.
5. Limites numéricos do sandbox PDF.
6. Política para horário local inexistente/ambíguo em DST.
7. Retenção por entidade e requisitos legais.
8. Forma de assinatura/provenance adotada no pipeline.
9. Se tarefas de Textract/parser/Bedrock terão Lambdas separadas já no primeiro release.

Essas escolhas são Type 1 quando afetam segurança, privacidade, contrato externo ou recuperação e devem seguir o protocolo de debate.
