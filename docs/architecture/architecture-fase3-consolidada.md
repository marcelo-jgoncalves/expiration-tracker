...
Status: consenso de conteúdo (Rodadas 1–3) + fechamento de ADRs materialmente relevantes (Rodada 4) — aguardando reavaliação de nota sob a rubrica de Design Maturity Score corrigida (ver `requirements.md` seção 13.1).

## Nota sobre avaliação (correção metodológica da Rodada 3)
A primeira avaliação de nota (Claude ~8,85 / Codex 5,9) aplicou a rubrica original de `requirements.md` seção 13.1, que exige evidência de implementação/teste para nota ≥7 — inaplicável a um artefato de design conceitual pré-implementação (o Implementation Blueprint só existe *depois* da arquitetura aprovada, seção 60 do prompt mestre). Claude e Codex corrigiram por consenso: `requirements.md` seção 13.1 agora define duas rubricas — **(A) Design Maturity Score** para checkpoints conceituais como este, e **(B) Operational Evidence Score** para o Gate de Aprovação Final (seção 23) sobre o sistema construído. Codex reavaliou sob a rubrica (A) e deu **8,1** (ainda abaixo de 9,0, por ADRs materialmente relevantes em aberto). A Rodada 4 abaixo fecha os ADRs mais relevantes para nova reavaliação.
Base: `docs/architecture/claude-architecture-proposal.md`, `docs/architecture/codex-architecture-proposal.md`, `docs/architecture/requirements.md`, `docs/architecture/capacity-model.md`.
...
**WAF**: condicional, não incondicional — obrigatório antes do lançamento em produção pública ou se surgir exigência de controle gerenciado; compatibilidade exata com API Gateway HTTP API a validar em ADR (nota técnica: WAF é suportado em HTTP API via associação direta, mas o comportamento de regras deve ser confirmado antes de assumir paridade total com REST API).
...
Itens que **permanecem abertos** (não fecháveis nesta fase sem mais pesquisa/decisão de produto): #2 (tipo de Step Functions), #5 (WAF×HTTP API), #6 (BSP WhatsApp), #7 (modelo Bedrock), #8 (auto-ajuste de shard fica manual por decisão consciente acima, não pendência), #9 (SLA de latência quarantine→clean, depende de `slo.md`), #10 (outbox — já resolvido na decisão 11 com sweeper), #11 (Step Functions desde Stage 1 — mesma pendência do #2), #12 (contrato WhatsApp — envelope já decidido, payload específico pendente de escolha de BSP #6).
...
8. Mecanismo de auto-ajuste ou alarme de partition-throttle para o número de shards por minuto no reminder scheduling (crítica do Claude, Rodada 2 — ver `round2-claude-critique.md`).
