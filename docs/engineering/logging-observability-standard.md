...
| 4 | Tracing distribuído & junção log-trace | 10% | Tracing distribuído | `correlationId` sobrevive a toda fronteira real de processo (SQS `MessageAttribute`, invocação Step Functions, chamada síncrona Lambda→Lambda); span/trace ADOT/X-Ray permanece correlacionável ao mesmo `correlationId` do log via `xrayTraceId`/`xraySampled` estruturados (`src/shared/observability/xray-trace-header.ts`, parseado de `_X_AMZN_TRACE_ID`, nunca o header bruto); um operador consegue, a partir de UM `correlationId`, encontrar tanto os logs quanto o trace de uma requisição real de ponta a ponta. **Nota de cardinalidade**: `correlationId`↔`xrayTraceId` não é 1:1 — um único fluxo lógico de negócio atravessa múltiplas invocações Lambda reais, cada uma com seu próprio trace X-Ray; a junção é uma relação observacional, nunca uma identidade equivalente. **Nota de sampling**: `xraySampled: false` ao lado de um `xrayTraceId` válido é comportamento normal de sampling (a invocação não tem necessariamente um trace persistido/consultável), não falha da integração. |
