...
| Editar `infra-terraform/`, `.github/workflows/{ci,cd}.yml`, rodar `terraform`/`aws` fora de sessão explicitamente autorizada | Proibido por padrão — exige instrução explícita da sessão | Precedente: instruções de sessão 2026-08-19/20 (trabalho ativo de infra por outra sessão) |
