...
>
> **Addendum (2026-08-28, sessão posterior à síntese original)**: o gate #2 abaixo (`GTR-01`/`W5-01`, guest trust/identidade do solicitante) **fechou** — ver `pilot-readiness-program.md`'s seção W5-01 e `decisions-log.md` D-060. O gate #3 (evidência operacional, Wave 2) **fechou por completo, mesma data**: todos os 6 drills (W2-03 a W2-08) executados com evidência real contra `dev` — kill switch, pipeline de lembretes fim a fim, DLQ/replay, restore real (RTO ≈3min44s) e load test real (977 invocações, cota real segurando sob carga, latência p99<2s) — só a metade credential-compromise do W2-08 não foi exercitada (achado de processo menor, não bloqueante). Ver `pilot-readiness-program.md`'s seção Wave 2 para o detalhe completo. O texto abaixo não foi reescrito (registro histórico do estado no momento da síntese) — só o gate #1 (`W3-06`, retenção/purga) continua real e aberto sem nenhum progresso desde a síntese original.
