...
>
> **Addendum (2026-08-28, sessão posterior à síntese original)**: o gate #2 abaixo (`GTR-01`/`W5-01`, guest trust/identidade do solicitante) **fechou** — ver `pilot-readiness-program.md`'s seção W5-01 e `decisions-log.md` D-060. O gate #3 (evidência operacional, Wave 2) **fechou quase por completo, mesma data**: W2-03/W2-04/W2-05/W2-06 executados com evidência real contra `dev` (kill switch, pipeline de lembretes fim a fim, DLQ/replay, restore real com RTO medido ≈3min44s), W2-08 parcialmente (validação de alarme real feita, credential-compromise em si não exercitado) — só resta W2-07 (load test, P2). Ver `pilot-readiness-program.md`'s seção Wave 2 para o detalhe completo. O texto abaixo não foi reescrito (registro histórico do estado no momento da síntese) — só o gate #1 (`W3-06`, retenção/purga) continua real e aberto sem nenhum progresso desde a síntese original.
