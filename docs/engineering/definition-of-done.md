...
- Não é um mecanismo *bloqueante* (sem lint/CI/hook dedicado) — continua sendo uma disciplina que o próprio agente escolhe aplicar antes de fechar o item. **Emenda (Marcelo, 2026-09-18)**: existe hoje uma Skill (`/task-checklist`, `.claude/skills/task-checklist/SKILL.md`) que opera `task-completion-checklist.md` (a versão checkbox deste documento + `change-risk-scale.md`/`quality-gate-tiers.md`/`joint-review-criteria.md`) de forma rápida e consistente — mas é invocada pelo próprio agente, nunca forçada por hook/CI. Isto reduz o atrito de aplicar a disciplina sem virar automação bloqueante; não é o mesmo passo que um hook que impede `completed` sem a linha `DoD:`, que continua reservado para se/quando houver evidência de que a Skill sozinha não é suficiente — mesmo princípio de `principles.md` #1 (não adicionar mecanismo antes de evidência do caso mais simples não bastar).
