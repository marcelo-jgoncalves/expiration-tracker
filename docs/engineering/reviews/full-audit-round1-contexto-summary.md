...
### Escopo maior que correção pontual, não impedimento externo — resolvido em sessão seguinte (commit `ddf4bc9`)

Marcelo pediu explicitamente para tratar esses 3 itens depois de ver o resumo acima. Resolvido no mesmo dia, fora da rodada formal de nota (sem 4ª rodada de nota cega — mudança mecânica/estrutural, verificável por leitura direta):

- **Context Routing & Progressive Disclosure** (#3, 15%) e **Clareza de Papéis & Proporcionalidade** (#2, 9%, parcial): `docs/engineering/README.md` criado — índice + tabela de roteamento por tipo de tarefa, mesmo padrão de `docs/architecture/README.md`. `AGENTS.md` §2 atualizado para apontar para ele.
- **Auditabilidade & Enforcement do Sistema de Contexto** (#9, 9%): `scripts/check-doc-drift.ts` (`npm run check-docs`) criado — link relativo quebrado + referência `AGENTS.md §N` desatualizada, bloqueante no CI (`guardrails`). `AGENTS.md` §6 atualizado para descrever o que a automação cobre vs. o que continua manual.
- **Higiene de Contexto & Sinal-Ruído** (#7, 8%): não higienizado por completo (risco de perda de evidência histórica genuína permanece real, avaliação não mudou) — mas `docs/engineering/README.md` agora classifica explicitamente os transcripts brutos com mojibake como evidência não-autoritativa e aponta sempre para o `-summary.md` correspondente, que é a mitigação de menor risco disponível sem reescrita byte-a-byte.

Nenhum desses 3 achados teve reavaliação formal de nota (Claude nem Codex) após esta correção — mesma decisão já tomada nas correções anteriores do eixo, registrada aqui por transparência, não por ter fechado o gate.
