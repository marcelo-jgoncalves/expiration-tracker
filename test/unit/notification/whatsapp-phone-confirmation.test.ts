...
describe("buildWhatsAppPhoneConfirmation", () => {
  it("builds a well-formed entity with a hashed code and a TTL-bound expiresAt", () => {
    const record = buildWhatsAppPhoneConfirmation({ tenantId: "t1", userId: "u1", phoneE164: "+15551234567", code: "123456", pepper: PEPPER, now: NOW, version: 1 });
  ...
  it("throws ValidationError before constructing anything, given a malformed phone number", () => {
    expect(() => buildWhatsAppPhoneConfirmation({ tenantId: "t1", userId: "u1", phoneE164: "not-a-phone", code: "123456", pepper: PEPPER, now: NOW, version: 1 })).toThrow(
      ValidationError,
    );
  });
});
