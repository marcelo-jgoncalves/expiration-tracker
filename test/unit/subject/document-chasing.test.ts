...
describe("document-chasing domain — GSI3 discriminator (D-046: shared index, non-overlapping SK shape)", () => {
  it("chasingGsi3Keys produces a GSI3SK shape that never matches the reminder parser", () => {
    const keys = chasingGsi3Keys({ tenantId: AUTH_TENANT, occurrenceId: "occ-1", scheduledAt: "2026-08-23T12:03:00.000Z", shardCount: 4 });
  ...
  it("buildChasingClaimGsi6Sk is ordered by claimExpiresAt (same sort-key discipline as reminders)", () => {
    const earlier = buildChasingClaimGsi6Sk("2026-08-23T12:00:00.000Z", AUTH_TENANT, "occ-1");
    const later = buildChasingClaimGsi6Sk("2026-08-23T13:00:00.000Z", AUTH_TENANT, "occ-2");
  ...
  it("documentChasingOccurrenceKey/documentChasingIntentKey co-locate under the subject partition, nested under assignment/documentRequest", () => {
    const occKey = documentChasingOccurrenceKey(AUTH_TENANT, "s1", "a1", "d1", "2026-08-23T12:00:00.000Z", "occ-1");
    ...
    const intentKey = documentChasingIntentKey(AUTH_TENANT, "s1", "a1", "d1", "intent-1");
