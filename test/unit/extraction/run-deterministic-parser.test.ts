...
describe("runDeterministicParser", () => {
  it("happy path: finds a confident candidate, does not need Bedrock, reports AI_EXTRACTION flag", async () => {
    ...
    const featureFlags = new FakeFeatureFlagsReader({ AI_EXTRACTION: true, OCR: true, WHATSAPP: false, EXTRACTION_DOCUMENT_ARCHIVE_TRIGGER_ENABLED: false, DOCUMENT_ARCHIVE_PROMOTION_ENABLED: false, WHATSAPP_DELIVERY_WORKER_ENABLED: false });
