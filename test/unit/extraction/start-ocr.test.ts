...
describe("startOcr", () => {
  ...
  it("throws OcrDisabledError when the OCR kill switch is off", async () => {
    ...
    await expect(
      startOcr(
        {
          featureFlags: new FakeFeatureFlagsReader({ AI_EXTRACTION: true, OCR: false, WHATSAPP: false, EXTRACTION_DOCUMENT_ARCHIVE_TRIGGER_ENABLED: false, DOCUMENT_ARCHIVE_PROMOTION_ENABLED: false, WHATSAPP_DELIVERY_WORKER_ENABLED: false }),
          quota,
          textract,
          jobs,
          tokenEncryptor: new FakeTaskTokenEncryptor(),
