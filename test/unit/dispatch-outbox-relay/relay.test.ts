...
describe("DispatchOutboxRelay / OutboxSweeperReminderDispatch core logic (M3.5)", () => {
  it("publishes a pending record with the right destination", async () => {
    ...
    const deps: RelayDeps = {
      store,
      sendToDispatchQueue: async (payload) => {
        sent.push(payload);
      },
  ...
  it("routing exclusivity: never touches a record without the SQS_REMINDER_DISPATCH_V1 destination", async () => {
    const store = new FakeRelayStore();
    const sent: unknown[] = [];
    const deps: RelayDeps = {
      store,
  ...
  it("two concurrent relays racing the same record: only one publishes (lease contention)", async () => {
    ...
    const depsA: RelayDeps = { store, sendToDispatchQueue: async (p) => void sent.push(p), now: () => "2026-08-19T10:00:05.000Z", leaseOwner: "relay-A" };
    const depsB: RelayDeps = { store, sendToDispatchQueue: async (p) => void sent.push(p), now: () => "2026-08-19T10:00:05.000Z", leaseOwner: "relay-B" };
  ...
  it("sweeper: publishes every pending SQS_REMINDER_DISPATCH_V1 candidate returned by the store", async () => {
    ...
    const result = await sweepPendingDispatch({
      store,
      sendToDispatchQueue: async (p) => void sent.push(p),
