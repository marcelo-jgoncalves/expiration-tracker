...
class FakeRelayStore implements OutboxRelayStore {
  ...
  // 2026-09-19 hot-partition fix (docs/architecture/reviews/outbox-sweeper-shared-partition/
  // PROPOSAL.md): the real store no longer filters by destination server-side - it can't,
  // there's only one shared GSI6 partition - so this fake returns everything, same as
  // production, and counts calls so the sweeper's "exactly one query" contract is testable.
  listCalls = 0;
...
describe("DispatchOutboxRelay / OutboxSweeperReminderDispatch core logic (M3.5)", () => {
  ...
  // Required by the approved fix (PROPOSAL.md §3): the shared GSI6 partition must be queried
  // exactly once per sweeper execution no matter how many destinations are swept - catches a
  // regression back to the old per-destination loop, which re-scanned the same partition once
  // per destination and timed out the Lambda under any real backlog.
  it("2026-09-19 hot-partition fix: queries the shared GSI6 partition exactly once, regardless of destinations count", async () => {
    const store = new FakeRelayStore();
    store.pending = [
      makeRecord({ SK: "EVENT#...#evt-1", eventId: "evt-1", destination: "SQS_REMINDER_DISPATCH_V1" }),
      makeRecord({ SK: "EVENT#...#evt-2", eventId: "evt-2", GSI6SK: "x2", destination: "SQS_NOTIFICATION_EMAIL_V1" }),
      makeRecord({ SK: "EVENT#...#evt-3", eventId: "evt-3", GSI6SK: "x3", destination: "SQS_REMINDER_DISPATCH_V1" }),
    ];
    await sweepPendingDispatch({
      store,
      senders: { SQS_REMINDER_DISPATCH_V1: async () => {}, SQS_NOTIFICATION_EMAIL_V1: async () => {} },
      now: () => "2026-08-19T10:05:00.000Z",
      leaseOwner: "sweeper-1",
    });
    expect(store.listCalls).toBe(1);
  ...
  // Required by the approved fix (PROPOSAL.md §8, Antigravity round 2 finding): publishOne
  // alone only excludes by `senders` (adapters this deps recognizes), never by a caller's
  // narrower `destinations` subset - the sweeper itself must filter candidates against it
  // BEFORE calling publishOne, or a caller asking to sweep only one destination would silently
  // also process every other destination its `senders` map happens to support.
  it("2026-09-19 hot-partition fix: a narrower `destinations` subset excludes other candidates without ever calling publishOne for them", async () => {
    const store = new FakeRelayStore();
    store.pending = [
      makeRecord({ SK: "EVENT#...#evt-1", eventId: "evt-1", destination: "SQS_REMINDER_DISPATCH_V1" }),
      makeRecord({ SK: "EVENT#...#evt-2", eventId: "evt-2", GSI6SK: "x2", destination: "SQS_NOTIFICATION_EMAIL_V1" }),
    ...
    const result = await sweepPendingDispatch({
      store,
      senders: {
        SQS_REMINDER_DISPATCH_V1: async () => { throw new Error("must never be called for this destination"); },
        SQS_NOTIFICATION_EMAIL_V1: async (p) => void emailSent.push(p),
      },
      destinations: ["SQS_NOTIFICATION_EMAIL_V1"],
      now: () => "2026-08-19T10:05:00.000Z",
      leaseOwner: "sweeper-1",
    });
    expect(result).toEqual({ attempted: 1, published: 1, failed: 0, stillPending: 0 });
    expect(emailSent).toHaveLength(1);
    expect(store.published.size).toBe(1);
  });

  it("causality (m5-observability-design.md #2/#5): the SendMessage sender receives the OutboxRecord's own correlationId, not one generated by the relay", async () => {
    const store = new FakeRelayStore();
    const seenCorrelationIds: string[] = [];
    const deps: RelayDeps = {
      store,
      senders: {
        SQS_REMINDER_DISPATCH_V1: async (_payload, correlationId) => {
          seenCorrelationIds.push(correlationId);
        },
      },
      now: () => "2026-08-19T10:00:05.000Z",
      leaseOwner: "relay-1",
    };
    const outcome = await publishOne(deps, makeRecord({ correlationId: "cor_original" }));
    expect(outcome.kind).toBe("PUBLISHED");
    expect(seenCorrelationIds).toEqual(["cor_original"]);
  });

  it("fallback (pre-M5 records without correlationId): falls back to eventId, never throws", async () => {
    const store = new FakeRelayStore();
    const seenCorrelationIds: string[] = [];
    const deps: RelayDeps = {
      store,
    ...
    const outcome = await publishOne(deps, makeRecord({ correlationId: undefined, eventId: "evt-legacy" }));
    expect(outcome.kind).toBe("PUBLISHED");
    expect(seenCorrelationIds).toEqual(["evt-legacy"]);
  });

  it("runs the send under the record's own correlationId context (isolation, one runWithContext per record)", async () => {
    const store = new FakeRelayStore();
    const contextsSeenInsideSend: Array<string | undefined> = [];
    const deps: RelayDeps = {
      store,
      senders: {
        SQS_REMINDER_DISPATCH_V1: async () => {
          contextsSeenInsideSend.push(getContext()?.correlationId);
        },
      },
      now: () => "2026-08-19T10:00:05.000Z",
      leaseOwner: "relay-1",
    };
    await publishOne(deps, makeRecord({ correlationId: "cor_a", SK: "EVENT#...#evt-1", eventId: "evt-1" }));
    await publishOne(deps, makeRecord({ correlationId: "cor_b", SK: "EVENT#...#evt-2", eventId: "evt-2", GSI6SK: "x2" }));
    expect(contextsSeenInsideSend).toEqual(["cor_a", "cor_b"]);
