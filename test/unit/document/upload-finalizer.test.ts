...
import type { DocumentStore } from "../../../src/modules/document/ports/document-store.js";
...
describe("finalizeUpload", () => {
  ...
  it("real bug found via Camada 3 (2026-08-22): retries and still confirms when MalwareResultWorker's own evidence write races in between this worker's read and write, instead of giving up as IGNORED_STALE", async () => {
    ...
    const racingStore: DocumentStore = {
      get: async <T,>(key: Parameters<DocumentStore["get"]>[0]) => {
        getCalls += 1;
        const doc = await store.get<Document>(key);
        if (getCalls === 1 && doc) {
          // Simulate MalwareResultWorker concurrently persisting its own evidence between our
          // read and our write - bumps version, so our transactWrite below (still holding the
          // stale version from THIS read) must fail once and retry.
          await store.update({ ...doc, malwareEvidence: { object: OBJECT, status: "NO_THREATS_FOUND", scanResultId: "scan-1", observedAt: "2026-08-22T00:00:01.000Z" }, version: 2 });
        }
    ...
    const outcome = await finalizeUpload(
      { store: racingStore, objects: fakeObjects(), parser: fakeParser(), tableName: TABLE, cleanBucket: CLEAN_BUCKET },
      { tenantId: "t1", itemId: "item1", documentId: "doc1", object: OBJECT },
    );

    expect(outcome).toBe("CONFIRMED");
    expect(getCalls).toBeGreaterThanOrEqual(2); // proves the retry actually happened, not a lucky first pass.
    const doc = store.allItems().find((i) => i["entityType"] === "Document") as unknown as Document | undefined;
