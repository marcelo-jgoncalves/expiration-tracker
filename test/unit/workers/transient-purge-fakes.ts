 ...
 * other migrated worker's own fake evaluator (not shared, different port shape). */
...
export class FakeTransientPurgeCandidateSource implements TransientPurgeCandidateSource {
   ...
   * `derive*MaintenanceDue()`/`transientPurgeGsi8Keys()`, same as every real writer does. A
   * `RESERVED` UploadSlot gets NO pointer (mirrors `deriveUploadSlotMaintenanceDue` returning
   * `undefined`), matching every real writer's behavior. Pass `GSI8PK`/`GSI8SK` explicitly in
   * `overrides` (via seedRaw) to simulate a hand-crafted/stale pointer a test wants to exercise
   * directly. */
  ...
  /** Escape hatch to seed a row with an explicit GSI8PK/GSI8SK regardless of what the real
   * derivation would produce — for the stale-pointer self-heal test. */
  seedRaw(item: Record<string, unknown> & EntityKey): void {
    this.items.set(k(item), { ...item });
  }
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<TransientGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#TRANSIENT" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
  ...
  getCandidate(key: EntityKey): Promise<TransientPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as TransientPurgeCandidate | undefined);
  }
