...
} from "../../../src/workers/quota-telemetry-purge/candidate-source.js";
 ...
 * `invitation-purge-fakes.ts`'s own evaluator (not shared, different port shape). */
 ...
 * `invitation-purge-fakes.ts`'s `FakeInvitationPurgeCandidateSource` exactly. */
export class FakeQuotaTelemetryPurgeCandidateSource implements QuotaTelemetryPurgeCandidateSource {
   ...
   * `deriveQuotaTelemetryMaintenanceDue()`/`quotaTelemetryGsi8Keys()`, same as `consume()`/
   * `incrementTelemetryCounter()` do inside their own writes. Pass `GSI8PK`/`GSI8SK` explicitly
   * in `overrides` to simulate a hand-crafted pointer a test wants to exercise directly. */
  seed(item: QuotaTelemetryPurgeCandidate): void {
    const due = deriveQuotaTelemetryMaintenanceDue(item);
    const gsi8 = quotaTelemetryGsi8Keys({ dueAtIso: due.dueAtIso, tenantId: item.tenantId, entityType: item.entityType, sk: item.SK });
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<QuotaTelemetryGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#QUOTA_TELEMETRY" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
    ...
    return Promise.resolve({
      items: page.map((i) => ({ PK: i.PK, SK: i.SK, dueAtIso: (i["GSI8SK"] as string).split("#TENANT#")[0]!, tenantId: i["tenantId"] as string })),
      lastEvaluatedKey,
    });
  }

  getCandidate(key: EntityKey): Promise<QuotaTelemetryPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as QuotaTelemetryPurgeCandidate | undefined);
  }
