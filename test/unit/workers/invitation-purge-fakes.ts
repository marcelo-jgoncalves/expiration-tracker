 ...
 * `membership-purge-fakes.ts`'s own evaluator (not shared, different port shape). */
 ...
 * `membership-purge-fakes.ts`'s `FakeMembershipPurgeCandidateSource` exactly. */
export class FakeInvitationPurgeCandidateSource implements InvitationPurgeCandidateSource {
  ...
  /** Seeds an Invitation row exactly as the real writer would leave it — computes GSI8PK/GSI8SK
   * itself via `deriveInvitationMaintenanceDue()`/`invitationGsi8Keys()` whenever the row is a
   * real candidate, same as `create-invitation.ts`/`revoke-invitation.ts` do inside their own
   * transaction. Pass `GSI8PK`/`GSI8SK` explicitly in `overrides` to simulate a stale/hand-crafted
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<InvitationGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#INVITATION_PURGE" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
  ...
  getInvitation(key: EntityKey): Promise<InvitationPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as InvitationPurgeCandidate | undefined);
  }
