import { deriveSecurityAuditMaintenanceDue, securityAuditGsi8Keys } from "../../../src/shared/security-audit-gsi8.js";
 ...
 * `quota-telemetry-purge-fakes.ts`'s own evaluator (not shared, different port shape). */
 ...
 * `quota-telemetry-purge-fakes.ts`'s `FakeQuotaTelemetryPurgeCandidateSource` exactly. */
export class FakeSecurityAuditPurgeCandidateSource implements SecurityAuditPurgeCandidateSource {
   ...
   * `deriveSecurityAuditMaintenanceDue()`/`securityAuditGsi8Keys()`, same as every `build*Event()`
   * does inside its own construction. Pass `GSI8PK`/`GSI8SK` explicitly in `overrides` to simulate
   * a hand-crafted pointer a test wants to exercise directly. */
  seed(item: SecurityAuditPurgeCandidate): void {
    const due = deriveSecurityAuditMaintenanceDue({ occurredAt: item.occurredAt });
    const gsi8 = securityAuditGsi8Keys({ dueAtIso: due.dueAtIso, tenantId: item.tenantId, entityType: item.entityType, sk: item.SK });
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<SecurityAuditGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#SECURITY_AUDIT" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
    ...
    return Promise.resolve({
      items: page.map((i) => ({
        ...
        entityType: i["entityType"] as SecurityAuditPurgeCandidate["entityType"],
      })),
      lastEvaluatedKey,
    });
  }
  ...
  getCandidate(key: EntityKey): Promise<SecurityAuditPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as SecurityAuditPurgeCandidate | undefined);
  }
