...
} from "../../../src/workers/membership-purge/candidate-source.js";
 ...
 * `AND`, `attribute_exists(x)`, `name = value`) — same deliberately-narrow-scope choice as every
 * other purge worker's fake in this test suite. */
 ...
 * mirroring the real SDK's `TransactionCanceledException` shape). */
export class FakeMembershipPurgeCandidateSource implements MembershipPurgeCandidateSource {
  ...
  /** Seeds a Membership row exactly as the real writer would leave it — computes GSI8PK/GSI8SK
   * itself via `deriveMembershipMaintenanceDue()`/`membershipGsi8Keys()` whenever the row is
   * REMOVED with a `removedAt`, same as `remove-membership.ts`/`leave-organization.ts` do inside
   * their own transaction. Pass `GSI8PK`/`GSI8SK` explicitly in `overrides` to simulate a stale/
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<MembershipGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#MEMBERSHIP_PURGE" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
    ...
    return Promise.resolve({
      items: page.map((i) => ({ PK: i.PK, SK: i.SK, dueAtIso: (i["GSI8SK"] as string).split("#TENANT#")[0]!, tenantId: i["organizationId"] as string })),
      lastEvaluatedKey,
    });
  }

  getMembership(key: EntityKey): Promise<MembershipPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as MembershipPurgeCandidate | undefined);
  }
  ...
  transactWrite(entries: TransactWriteEntry[]): Promise<void> {
    ...
    const failedIndex = entries.findIndex((entry, i) => {
      void i;
