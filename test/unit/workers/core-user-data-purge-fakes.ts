...
} from "../../../src/workers/core-user-data-purge/candidate-source.js";
 ...
 * `delivery-record-purge-fakes.ts`'s own evaluator (not shared, different port shape). */
 ...
 * `delivery-record-purge-fakes.ts`'s `FakeDeliveryRecordPurgeCandidateSource` exactly, adapted
 * for the `deletedAt`-gated derive function (a row with no `deletedAt` gets NO pointer at seed
 * time, unlike delivery-record's always-due-eventually shape). */
export class FakeCoreUserDataPurgeCandidateSource implements CoreUserDataPurgeCandidateSource {
   ...
   * `deriveCoreUserDataMaintenanceDue()`/`coreUserDataGsi8Keys()`, same as the real write path
   * does. A row whose `deletedAt` is absent gets NO GSI8 pointer at all (never a candidate). */
  seed(item: CoreUserDataPurgeCandidate): void {
    const due = deriveCoreUserDataMaintenanceDue({ deletedAt: item.deletedAt });
    const gsi8 = due.dueAtIso ? coreUserDataGsi8Keys({ dueAtIso: due.dueAtIso, tenantId: item.tenantId, entityType: item.entityType, sk: item.SK }) : {};
  ...
  queryDue(input: { before: string; exclusiveStartKey?: Record<string, unknown> }): Promise<CoreUserDataGsi8Page> {
    const all = [...this.items.values()]
      .filter((i) => i["GSI8PK"] === "WORK#CORE_USER_DATA" && typeof i["GSI8SK"] === "string" && (i["GSI8SK"] as string) < input.before)
    ...
    return Promise.resolve({
      items: page.map((i) => ({
        ...
        entityType: i["entityType"] as CoreUserDataPurgeCandidate["entityType"],
      })),
      lastEvaluatedKey,
    });
  }
  ...
  getCandidate(key: EntityKey): Promise<CoreUserDataPurgeCandidate | undefined> {
    return Promise.resolve(this.items.get(k(key)) as unknown as CoreUserDataPurgeCandidate | undefined);
  }
