 ...
 * builds — same evaluator shape as `core-user-data-purge-fakes.ts`'s (narrow support for exactly
 * what `purge.ts` emits: `attribute_exists`/`=`, top-level `AND`), deliberately not shared with
 * it (different port shape). */
...
export class FakeDeliveryRecordPurgeCandidateSource implements DeliveryRecordPurgeCandidateSource {
  ...
  scanCandidates(exclusiveStartKey?: Record<string, unknown>): Promise<DeliveryRecordScanPage> {
    const all = [...this.items.values()].filter(
      (i) => (i["entityType"] === "NotificationIntent" || i["entityType"] === "NotificationAttempt") && i["createdAt"] !== undefined,
    ) as unknown as DeliveryRecordPurgeCandidate[];
