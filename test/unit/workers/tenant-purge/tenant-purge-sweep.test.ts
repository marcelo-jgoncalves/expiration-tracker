...
function record(tenantId: string, status: TenantLifecycleStatus, updatedAt: string): TenantLifecycleRecord {
  return { ...tenantLifecycleKey(tenantId), SK: "LIFECYCLE", entityType: "TenantLifecycleRecord", tenantId, status, createdAt: "2026-01-01T00:00:00.000Z", updatedAt, version: 1 };
}
...
function recordingStarter(): TenantPurgeExecutionStarter & { started: string[] } {
  ...
  return { started, startExecution: async (input) => void started.push(input.name) };
}

/** Empty stores: every verification pass reports a fully converged tenant unless a test overrides
 * one of them. */
function baseDeps(lifecycle: TenantLifecycleScanSource, executions: TenantPurgeExecutionStarter): TenantPurgeSweepDeps {
  return {
    lifecycle,
    executions,
