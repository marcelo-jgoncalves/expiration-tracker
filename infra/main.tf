...
module "document_archive_guest_handler" {
  ...
  source_dir     = "${local.dist_dir}/document-archive-guest-handler"
  adot_layer_arn = var.adot_layer_arn
  environment_variables = merge(local.common_env, {
...
# DeliveryRecordPurgeWorker (D-152, docs/architecture/reviews/
# quarantine-retention-scoping/estado-final-consolidado.md, DELIVERY_RECORD row, Prioridade 2) -
# physically purges NotificationIntent/NotificationAttempt rows once createdAt+180d has passed,
# within ACTIVE tenants only (a completely separate mechanism from tenant-purge's
# full-tenant-closure pipeline) - see src/workers/delivery-record-purge/purge.ts's doc comment.
# Migrated off the base-table Scan onto GSI8 by D-179/D-18x (8th of 9 workers) -
# gsi8_read_policy_json/worker_transact_write_policy_json scoped to WORK#DELIVERY_RECORD/
# DLQ#DELIVERY_RECORD, same pattern as security_audit_purge_handler below (this worker also has a
...
module "delivery_record_purge_handler" {
  ...
  source_dir            = "${local.dist_dir}/delivery-record-purge-handler"
  adot_layer_arn        = var.adot_layer_arn
  timeout_seconds       = 300
  environment_variables = local.common_env
  policy_documents_json = [
    module.table.tenant_facing_read_write_policy_json,
    module.table.gsi8_read_policy_json["delivery_record_purge"],
    module.table.worker_transact_write_policy_json["delivery_record_purge"],
  ]
  tags = { Project = local.project_name, Environment = var.environment }
}

resource "aws_iam_role" "delivery_record_purge_schedule" {
  name = "${module.delivery_record_purge_handler.function_name}-schedule-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy" "delivery_record_purge_schedule_invoke" {
  name = "invoke"
  role = aws_iam_role.delivery_record_purge_schedule.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid      = "InvokeDeliveryRecordPurge"
        Effect   = "Allow"
        Action   = "lambda:InvokeFunction"
        Resource = module.delivery_record_purge_handler.live_alias_arn
      }
    ]
  })
}

resource "aws_scheduler_schedule" "delivery_record_purge" {
  name                         = "${local.name_prefix}-delivery-record-purge"
  schedule_expression          = "cron(30 5 * * ? *)" # daily 05:30 UTC, after core-user-data-purge (05:00)
  schedule_expression_timezone = "UTC"
  state                        = var.schedules_enabled ? "ENABLED" : "DISABLED"
  ...
  target {
    arn      = module.delivery_record_purge_handler.live_alias_arn
    role_arn = aws_iam_role.delivery_record_purge_schedule.arn
    # Literal string, NOT jsonencode() - see reminder-schedule/main.tf's header for the real
    # angle-bracket-escaping bug that rule exists to prevent.
    input = "{\"scheduledTime\":\"<aws.scheduler.scheduled-time>\"}"
  }
...
# SecurityAuditPurgeWorker (D-153, docs/architecture/reviews/
# quarantine-retention-scoping/estado-final-consolidado.md, SECURITY_AUDIT row, Prioridade 3) -
...
module "security_audit_purge_handler" {
  ...
  source_dir            = "${local.dist_dir}/security-audit-purge-handler"
  adot_layer_arn        = var.adot_layer_arn
  timeout_seconds       = 300
  environment_variables = local.common_env
  policy_documents_json = [
...
resource "aws_iam_role" "security_audit_purge_schedule" {
  name = "${module.security_audit_purge_handler.function_name}-schedule-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy" "security_audit_purge_schedule_invoke" {
  name = "invoke"
  role = aws_iam_role.security_audit_purge_schedule.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid      = "InvokeSecurityAuditPurge"
        Effect   = "Allow"
        Action   = "lambda:InvokeFunction"
        Resource = module.security_audit_purge_handler.live_alias_arn
      }
    ]
  })
}

resource "aws_scheduler_schedule" "security_audit_purge" {
  name                         = "${local.name_prefix}-security-audit-purge"
  schedule_expression          = "cron(0 6 * * ? *)" # daily 06:00 UTC, after delivery-record-purge (05:30)
  schedule_expression_timezone = "UTC"
  state                        = var.schedules_enabled ? "ENABLED" : "DISABLED"
  ...
  target {
    arn      = module.security_audit_purge_handler.live_alias_arn
    role_arn = aws_iam_role.security_audit_purge_schedule.arn
    # Literal string, NOT jsonencode() - see reminder-schedule/main.tf's header for the real
    # angle-bracket-escaping bug that rule exists to prevent.
    input = "{\"scheduledTime\":\"<aws.scheduler.scheduled-time>\"}"
  }
