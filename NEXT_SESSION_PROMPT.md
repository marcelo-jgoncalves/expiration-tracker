...
11. **Frontend completo do P0** — 🟡 planejamento + auditoria de qualidade + plano de sequenciamento CONCLUÍDOS (25/25 telas); **Bloco 0 (fundação) IMPLEMENTADO e VERIFICADO (D-254, 2026-09-10)**. Plano de 25 telas (D-247, `docs/frontend/p0-screen-inventory-plan.md`); os 3 gaps de backend/BFF fechados (D-248); as 24 specs concretas do Claude Design foram TODAS auditadas e revisadas (D-251) — 24/24 NOT PASS pré-revisão, corrigidas na própria auditoria; a 25ª (A10) foi escrita do zero e auditada em 2026-09-10 (D-252), WORLD-CLASS-READY de primeira. Plano de sequenciamento CONVERGIDO (D-253) — 11 blocos ordenados. **Bloco 0 (D-254)**: migração real de rotas para `/app/:orgId/...` (`OrgRouteGuard`/`LegacyOrgRedirect`/`useOrgPath`, caminhos antigos preservados como redirect), AppShell com navegação declarativa RBAC-aware (`shell/navigation.ts`), e os dois componentes de design system SLF-01 (`MetricCardGrid`, risk-prioritized) e SLF-05 (`GuestLinkUnavailable`) — todos reais, testados (204/204 unit+component, 35/35 E2E+visual, gate completo verde). Protocolo Claude↔Codex rodado 3x nesta rodada (2 tentativas não convergiram por limitação da ferramenta `codex exec` — gastou o orçamento relendo docs grandes do repo sem produzir veredito; a 3ª, com prompt reduzido e conteúdo inline, produziu "NEEDS FIXES" apontando 2 achados reais — nav RBAC escondendo "Membros" de papéis que a action real `membership:list-members`/READ_ONLY_ROLES permite ver, e o próprio `AppShell` ainda usando caminhos bare em vez de `useOrgPath()` — ambos corrigidos e revalidados; nota numérica formal ≥9.0 cega NÃO foi obtida desta vez, registrado honestamente em D-254, não substituído silenciosamente). **Bloco 1 — PARCIAL (D-255, 2026-09-10)**: A19 achado CRITICAL #3 (promoção/demoção de OWNER bloqueada para todo mundo) corrigido em `Members.tsx` (#1/#2 já estavam corrigidos desde D-254, verificado); integração de storage quota (A03 card condicional + seção A19) construída do zero (backend existia desde D-249, zero frontend antes disso) — ambos testados, no `develop` (commits `995dbdb`, `5d1ddcb`). **A01 e A02 NÃO reconciliados** — bloqueados por decisão de produto genuína, não por falta de tempo: A01's spec (formulário de e-mail/senha client-side) contradiz a arquitetura BFF/Cognito Hosted UI real e já auditada (`frontend-production-foundation.md` F1); A02's spec exige campos que não existem em `UsableOrganization`/`SessionInfo` (contagem operacional por org, `suspended`, convite pendente pré-seleção). Protocolo Claude↔Codex do bloco **adiado** até essas 2 decisões fecharem (ver D-255 para o detalhe completo e a pergunta exata para Marcelo). **PRÓXIMA AÇÃO**: (1) decisão de Marcelo sobre A01 (reconciliar a spec com o BFF real, ou declarar que A01 nunca precisa de tela client-side própria) e sobre os campos de dados que A02 precisa; (2) reconciliar o que já é possível em A02 sem dado novo; (3) protocolo Claude↔Codex no bloco; (4) só então Bloco 2.
