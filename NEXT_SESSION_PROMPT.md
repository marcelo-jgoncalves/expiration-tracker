...
3. **WhatsApp operacional** — 🟢 IMPLEMENTADO por completo do lado de engenharia (D-197/ADR-0012, fatias 1-4/5, D-245 fecha a fatia 4/5 — quota real de 24h D-8 + IAM dedicada `dynamodb:LeadingKeys=["WHATSAPP#PORTFOLIO"]`, commitado em `develop`, ainda NÃO mergeado em `main`/aplicado em `dev` — próxima sessão decide quando). Fatia 3/5 segue **VERIFICADA AO VIVO em `dev`** (D-242). **Restam apenas**: fatia 5/5 (router wiring final `SUPPORTED_CHANNELS`+kill switch+RBAC+terraform completo — nenhuma decisão nova, mecânica) e o bloqueio de produto/jurídico E-019 (aviso de privacidade, DPA Meta formalmente aceito, residência de dados decidida — nenhum feito ainda, fora do controle de engenharia; as credenciais reais da Meta no secret também dependem disso). O item P0 de engenharia pura fica pendente só de fatia 5/5 + o `terraform apply`/merge-para-`main` desta fatia 4/5.
...
1. Item 3 do P0 (WhatsApp operacional) — fatia 3/5 **verificada ao vivo (D-242)**; fatia 4/5 (quota 24h + IAM dedicada) **implementada e testada (D-245), commitada em `develop`, ainda não mergeada em `main`/aplicada em `dev`**. Falta fatia 5/5 (router wiring final + kill switch + RBAC + terraform completo, mecânica). Bloqueante à parte para uso com usuário real (E-019, item 5 da lista de pendências acima) segue fora do controle de engenharia pura.
