...
**PRÓXIMA AÇÃO EXPLÍCITA E IMEDIATA (D-250, 2026-09-09, substitui a frase genérica "aguardar sinal de Marcelo" para este passo específico)**: Marcelo revisou as 24 specs e avaliou que são funcionalmente boas mas "não tão profissionais e world-class" quanto ele gostaria — pediu uma auditoria rigorosa **antes de qualquer código de frontend**. Esta sessão construiu o instrumento (`docs/frontend/screen-spec-audit-rubric.md`, D-250: 8 eixos funcionais de D-247 + 8 eixos novos de excelência visual V1-V8, combinados 60/40, gates não-compensatórios) mas **NÃO auditou nenhuma das 24 specs ainda** — isso é o próximo passo, um por vez ou em lotes pequenos (ver "Subdivide Long Agent Tasks" na memória — não uma mega-tarefa única), produzindo um registro de auditoria por tela (template §6 do rubric) em `docs/architecture/reviews/` (pasta a nomear na hora, convenção igual a `p0-frontend-screens-scoping/`). Achados que apontem para uma limitação do próprio `design-system.md` (não apenas da spec) devem ser nomeados como tal (taxonomia SYSTEM GAP/SYSTEM CONSTRAINT/SYSTEM EVOLUTION CANDIDATE do rubric §5) — Marcelo foi explícito que o design system atual é precedente reconciliável, não cerca fixa. Só depois dessa auditoria (e de qualquer revisão de spec/design-system que ela recomende) a implementação real de frontend começa. **Pendência que só Marcelo pode resolver, registrada aqui, não bloqueia o início da auditoria**: os thresholds numéricos do rubric (VisualSpecScore ≥85/≥90, V7 ≥13.5/17) são explicitamente provisórios até calibrados contra os primeiros 5-8 screens auditados de verdade (rubric §4) — se o primeiro lote falhar sistematicamente no tier "world-class-ready", isso deve ser nomeado como possível recalibração de threshold, não silenciosamente ignorado nem usado para reprovar todo o lote sem discussão.
