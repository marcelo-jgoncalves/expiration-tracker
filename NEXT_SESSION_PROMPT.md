...
11. **Frontend completo do P0** — ❌ implementação explicitamente adiada por Marcelo (2026-09-04) — não iniciar código. **Planejamento CONCLUÍDO (D-247, 2026-09-09)**: protocolo Claude↔Codex em 2 etapas convergiu (Etapa 1 rubric 9,4/9,3; Etapa 2 screen inventory 9,4/9,3, ambas ≥9,0) num plano de 25 telas (23 autenticadas + 2 guest) em `docs/frontend/p0-screen-inventory-plan.md`, pronto para o Claude Design construir o protótipo. **3 gaps reais de backend/BFF nomeados como bloqueantes** (não meros adiamentos) para telas específicas: G2 nenhuma rota HTTP de listagem da Review Queue/Documents (bloqueia tela A13 por inteiro); G3 os 7 endpoints CSV de relatórios não estão no allowlist do proxy BFF (bloqueia download real da tela A16); G4 `docarchive:request-create` (solicitação avulsa) sem rota HTTP tenant-facing (bloqueia esse controle específico dentro da tela A14, série recorrente não afetada). Esses 3 gaps precisam fechar antes/durante a implementação das telas A13/A16/A14 especificamente — não bloqueiam o início da implementação das outras 22 telas.
...
**CONCLUÍDO (D-247, 2026-09-09)**: as duas etapas acima convergiram — `docs/frontend/p0-screen-inventory-plan.md` (25 telas, autocontido, ver D-247 no `decisions-log.md` e `docs/architecture/reviews/p0-frontend-screens-scoping/` para o trilho completo de evidência). Implementação do frontend continua **NÃO INICIADA** — aguardar sinal explícito de Marcelo antes de começar a construir qualquer tela. Quando ele sinalizar: (1) considerar fechar primeiro os 3 gaps de backend/BFF nomeados acima (G2/G3/G4) ou pelo menos sequenciar a implementação das telas A13/A16/A14 depois deles; (2) ler `docs/frontend/design-system.md` (Operational Calm, já `APPROVED`) antes de qualquer decisão visual — o plano de telas deliberadamente não tomou nenhuma decisão de layout/visual.
