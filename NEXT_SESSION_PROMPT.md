...
11. **Frontend completo do P0** — 🟡 planejamento + auditoria de qualidade + plano de sequenciamento CONCLUÍDOS (25/25 telas), implementação real do plano ainda NÃO INICIADA. Plano de 25 telas (D-247, `docs/frontend/p0-screen-inventory-plan.md`); os 3 gaps de backend/BFF fechados (D-248); as 24 specs concretas do Claude Design foram TODAS auditadas e revisadas (D-251) — 24/24 NOT PASS pré-revisão, corrigidas na própria auditoria; a 25ª (A10, nunca gerada pelo pacote original) foi escrita do zero e auditada em 2026-09-10 (D-252), WORLD-CLASS-READY de primeira. **Plano de sequenciamento de implementação CONVERGIDO (D-253, 2026-09-10)**: `docs/frontend/implementation-sequencing-plan.md` — 11 blocos ordenados, correção real verificada de que mais telas já têm código parcial do que a documentação anterior registrava (A02/A03/A08/A09/A19/A23), migração real `/app/:orgId` identificada como item real de Bloco 0 (não só spec). **PRÓXIMA AÇÃO EXPLICITAMENTE AUTORIZADA POR MARCELO, sem esperar confirmação**: iniciar a implementação real do Bloco 0 (`docs/frontend/implementation-sequencing-plan.md` §1) — migração de rotas para `/app/:orgId`, AppShell com navegação declarativa RBAC-aware, e os dois componentes de design system SLF-01 (metric-card priorizado por risco) e SLF-05 (`GuestLinkUnavailable`) — seguido do Bloco 1 (A01/A02/A03/A19). Rodar o protocolo Claude↔Codex novamente ao final de cada bloco, per o checklist de 10 itens do plano §4.
