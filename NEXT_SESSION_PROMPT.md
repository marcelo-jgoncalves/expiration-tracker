...
1. **Multi-User B2B — autorizado a prosseguir, Wave B2B-0 `DONE` (D-084/D-085, 2026-08-29).** Design técnico `APPROVED` (Claude 9,2/Codex 9,2, protocolo completo em `roadmap-evolution/17-multi-user-b2b-revised-strategy.md` §125) e timing decidido diretamente pelo Marcelo. **Wave B2B-0 (inventário read-only, `roadmap-evolution/17` §105) concluída** — ver `docs/architecture/multi-user-b2b-wave-b2b0-inventory.md`: os 3 pontos de origem de `tenantId=userId` confirmados (`bootstrap-identity.ts`, `bff-auth-service.ts`, `recipient-resolver.ts`, não homogêneos — 2 origens reais + 1 consumidor), suíte de testes contada de forma reproduzível (122 arquivos/1092 casos, não ~1104 como citado informalmente antes; padrão `tenantId=userId` em 10 arquivos/12 ocorrências, sem fixture centralizado), frontend com **zero** isolamento de tenant em query keys/cache hoje, e S3 com 3 prefixos reais distintos (diverge da suposição de prefixo único de §68). Achado colateral (não Multi-User B2B, ver tabela de gates abaixo): BFF login path (`bff-auth-service.ts`) não cria `TenantLifecycleRecord` e não tem fencing, ao contrário do path direto/API. **Próxima ação real**: Wave B2B-1 — rodada Claude↔Codex dedicada ao physical model (`roadmap-evolution/17` §106), ainda não iniciada. Ver `roadmap-evolution/17` §105-124 para as 16 waves completas (B2B-0 a B2B-15) e §121 para as 25 perguntas adversariais obrigatórias antes de aprovar o design físico final (ainda não fechado — só a direção está `APPROVED`, o "Proposta conceitual de physical model" §60 precisa passar por Claude↔Codex de novo quando for formalizado, per o próprio §60: "O design final deve passar por Claude↔Codex").
...
| BFF login path (`bff-auth-service.ts`) sem `TenantLifecycleRecord`/fencing, ao contrário do path direto (achado da Wave B2B-0, não é Multi-User B2B em si) | Decisão de quando corrigir — pode ser um chunk pequeno isolado, ou dentro de Wave B2B-2 (mesmos arquivos de bootstrap) | `docs/architecture/multi-user-b2b-wave-b2b0-inventory.md` §1.1 |
