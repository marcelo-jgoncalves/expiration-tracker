...
**CONCLUÍDO (D-247, 2026-09-09)**: as duas etapas acima convergiram — `docs/frontend/p0-screen-inventory-plan.md` (25 telas, autocontido, ver D-247 no `decisions-log.md` e `docs/architecture/reviews/p0-frontend-screens-scoping/` para o trilho completo de evidência). Os 3 gaps de backend/BFF (G2/G3/G4) foram fechados em D-248 (mesmo dia). Implementação do frontend continua **NÃO INICIADA** — aguardar sinal explícito de Marcelo antes de começar a construir qualquer tela. Quando ele sinalizar: (1) ler `docs/frontend/design-system.md` (Operational Calm, já `APPROVED`) antes de qualquer decisão visual — o plano de telas deliberadamente não tomou nenhuma decisão de layout/visual; nenhum gap de backend resta bloqueando A13/A14/A16 especificamente.
