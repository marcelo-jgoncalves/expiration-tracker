...
19. **Versionamento completo de `Document` (item-level) — SUSPENSO por Marcelo (2026-09-23)**: pedido original após comparar a tela "Documento" real com o protótipo (`Document` é 1 linha = 1 arquivo, sem histórico/versão). Investigação confirmou nível 5-6, estruturalmente equivalente ao D-143. Marcelo pausou por incerteza sobre se agrega valor real ao produto — não rodar o protocolo nem retomar sem pedido explícito dele. "Baixar documento" (a outra metade do pedido original) já foi implementado sem protocolo (D-313, nível 1-3, aditivo puro).
...
23. ~~Login/signup/reset de senha via UI própria (reversão de D-320)~~ — **APROVADO via protocolo Claude↔Codex (D-329, 2026-09-23/24)**: 3 rodadas, nota cega final Claude 9,3/Codex 9,1 (ambos ≥9,0). Achados reais corrigidos com código: login-CSRF via `requireSameSiteFetch()` (novo, 6 rotas D-3xx), corpo malformado (`parseJsonObjectBody()`), indisponibilidade de login virando falso "credencial inválida" (agora 503), anti-enumeração de `forgotPassword` (dobra `InvalidParameterException`), falso-sucesso de `confirmSignUp`/`resendConfirmationCode` (narrowing por mensagem, `looksAlreadyConfirmed()`), e um bug de integração real e confirmado: `device_configuration` no Cognito ativava device-remembering pool-wide (nunca usado por nenhum código, zero chamadas a `ConfirmDevice`) e quebrava o refresh de sessões `InitiateAuth` via `/oauth2/token` — **removido de `infra/modules/cognito/main.tf`**. **Risco residual nomeado, verificação pendente**: confirmar após o próximo deploy real que o pool `dev` (`aws cognito-idp describe-user-pool --profile claude-dev`) realmente limpou `DeviceConfiguration`, e exercitar um refresh real pós-expiração do access token. **Pendências REGISTRADAS, não bloquearam o fechamento** (o próprio Codex propôs essas saídas): (a) rate-limiting dedicado por conta/IP nas rotas de auth — só o throttle agregado de toda a API BFF (rate 10/burst 20) existe hoje; **obrigatório resolver antes de usuário real**, junto com E-019 (item 4). (b) `signUp` continua revelando e-mail já cadastrado (409) — decisão consciente e mantida (UX real do frontend + mesmo comportamento que a Hosted UI já tinha, nunca uma regressão). Detalhe completo: `decisions-log.md` D-329, `docs/architecture/reviews/d321-direct-auth-adversarial-review/`.
