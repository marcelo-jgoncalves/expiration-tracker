...
**Prioridade 1, pedido explícito de Marcelo (2026-09-08) — fatias 1/4, 2/4 e 3/4 CONCLUÍDAS (D-237/D-238/D-239)**: propagar `AuthorizedTenantId` (branded type criado em D-234, `src/modules/identity/domain/authorization.ts`) aos key-builders de persistência de TODOS os módulos. **document-archive** (D-237), **expiration** (D-238) e **subject** (D-239) já fechados: todo key-builder de domínio dos três módulos exige `AuthorizedTenantId`, `authorizedTenantIdFromPersistedEntity()` cobre o lado worker/SQS/guest-token/S3-GuardDuty sem `RequestContext`, zero casts novos fora de `authorization.ts`, suíte completa verde (2786/2786) nas três fatias. **Resta só organization** — mesmo padrão (D-237/D-238/D-239 documentam o design e os dois construtores a reusar), única fatia de sessão futura, mesma ordem de esforço (dezenas de call sites). Antes de começar: releia D-237/D-238/D-239 inteiros (o design já resolvido, não redecidir do zero) e confirme se algum key-builder desse módulo tem uma forma diferente de acesso worker/queue que precise de um terceiro construtor (nenhum caso assim apareceu em document-archive, expiration nem subject). Protocolo Claude↔Codex: dispensado nas três fatias já feitas (réplica mecânica de decisão de design já registrada) — mesmo critério vale para organization, salvo achado genuinamente novo.
