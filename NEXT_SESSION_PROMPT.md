...
**Prioridade 1, pedido explícito de Marcelo (2026-09-08) — fatia 1/4 CONCLUÍDA (D-237)**: propagar `AuthorizedTenantId` (branded type criado em D-234, `src/modules/identity/domain/authorization.ts`) aos key-builders de persistência de TODOS os módulos. **document-archive já fechado** (D-237): todo key-builder de domínio do módulo exige `AuthorizedTenantId`, novo `authorizedTenantIdFromPersistedEntity()` cobre o lado worker/SQS sem `RequestContext`, zero casts novos fora de `authorization.ts`, suíte completa verde (2786/2786). **Restam expiration/subject/organization** — mesmo padrão (D-237 documenta o design e os dois construtores a reusar), cada um sua própria fatia de sessão futura, mesma ordem de esforço (dezenas de call sites por módulo). Antes de começar cada fatia: releia D-237 inteiro (o design já resolvido, não redecidir do zero) e confirme se algum key-builder desse módulo tem uma forma diferente de acesso worker/queue que precise de um terceiro construtor (nenhum caso assim apareceu em document-archive). Protocolo Claude↔Codex: D-237 rodou 1 rodada (convergência forte, 9,3/9,4) em vez do mínimo de 3 — avalie se cada fatia seguinte precisa de rodada própria ou se pode reusar a decisão de design já registrada (é a mesma mudança estrutural, só um módulo diferente).
