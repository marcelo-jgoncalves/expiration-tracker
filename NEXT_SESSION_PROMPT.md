...
**Planejamento de interface**: 6 documentos/artefatos produzidos em sequência, todos `docs/frontend/*.md` (mais `prototype/` para o 6º), cada um fechado via protocolo Claude↔Codex e `APPROVED`:
...
Esses documentos descobriram **3 blockers técnicos reais de backend** (nenhum resolvido, todos citados por ID em todo lugar relevante, nunca mascarados):
...
**Próxima etapa natural do design** (ainda não iniciada): Heuristic Evaluation + Accessibility Evaluation + User Validation, usando o protótipo interativo real em `prototype/` (ver `prototype/README.md` para como rodar) e `interface-interaction-prototype.md` como input. Não obriga início imediato de Visual Design/Design System.

## Decisões pendentes do Marcelo

1. **Prioridade entre**: (a) implementar o Full BFF; (b) corrigir os 3 blockers de backend (BLOCKER-A/B/C); (c) continuar o design de interface (Heuristic + Accessibility Evaluation + User Validation); (d) iniciar M7 (extração/OCR). Nenhuma bloqueia as outras — podem avançar em paralelo (ver Engineering Enablement Dependencies em `interface-conceptual-model-and-information-architecture.md` §36).
...
- `docs/frontend/interface-quality-standard.md` ainda não existe como arquivo formal — os 6 documentos de interface usam os nomes de eixo direto do prompt-fonte. `expiration-tracker-bff-frontend-quality-standard.md` (raiz) contém uma rubrica candidata (§13-30) que, se adotada como padrão oficial, deveria passar pela mesma convergência independente Claude↔Codex que os 9 eixos de `docs/engineering/joint-review-criteria.md` já usaram — não decidido ainda.
...
- `docs/frontend/` — os 6 documentos de planejamento de interface aprovados; `prototype/` — o protótipo interativo executável (ver `prototype/README.md`).
