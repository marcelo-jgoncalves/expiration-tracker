...
**Não confie nesta seção sem confirmar.** `git branch --show-current` deve ser `develop`; `git log --oneline -5` e `git status` antes de assumir qualquer coisa abaixo como pendente ou concluído — múltiplas sessões/máquinas trabalham neste repo. Escrito com `develop` em sincronia com `main` logo após o merge do PR #98 (Wave B2B-4/D-094, commit `06f89a5`, CI verde).

## Fase atual

`Consolidation + Pilot Readiness` concluído com recomendação **CONDITIONAL GO** (`docs/engineering/pilot-readiness-assessment.md`). M0-M11 implementados e deployados; M7 (extração/OCR) code-complete e **E2E PROVEN** em `dev`; Full BFF + Frontend Production Foundation `APPROVED` e implementados; planejamento de interface 8/9 etapas `APPROVED` (só falta User Validation, em suspenso a pedido do Marcelo). W3-07 (exclusão física de tenant/DSR/LGPD) tem design `APPROVED WITH CONDITIONS` (D-066/D-067) e está em implementação incremental por chunks (D-068 a D-083). M12 (billing) bloqueado por decisão de fornecedor (D-052). **Multi-User B2B (D-084 a D-095): design técnico `APPROVED` via protocolo Claude↔Codex, physical model formalizado (Wave B2B-1, D-086) e timing decidido diretamente pelo Marcelo — proceder agora, supersedendo o gatilho comercial que antes gated M13.** Waves B2B-0 a B2B-4 `DONE`. **Wave B2B-5 (RequestContext Cutover) tem escopo `APPROVED` via protocolo Claude↔Codex 3 rodadas (D-095, `docs/architecture/multi-user-b2b-wave-b2b5-scope.md`), já apresentado ao Marcelo — implementação NÃO iniciada, aguardando o sinal dele para começar** (é a primeira wave a eliminar de fato o fallback `tenantId=userId` e mexer no fluxo de login real; pendência de confirmação dele, não de protocolo — ver "O que está em andamento" item 1). **Lembrete de calibração (D-093, `AGENTS.md` §1)**: projeto sem usuário real nem produção — não pesar "quebraria conta/sessão existente em `dev`" como risco bloqueador.
...
- **D-095 (2026-08-30) — Wave B2B-5 (RequestContext Cutover) com escopo `APPROVED`, implementação PENDENTE**: protocolo Claude↔Codex 3 rodadas (7,8/7,8 → 8,7/8,6 → 9,1/9,1), fecha 3 lacunas reais achadas por leitura do código (canal de transporte de `activeOrganizationId` inexistente; `Membership.role=ADMIN` sem tratamento na matriz de autorização real; `CreateOrganizationService` sem consumidor HTTP, o que deixaria todo login novo preso). Cap transacional em `POST /bff/organizations` via `GlobalUser.hasCreatedOrganization`. Detalhe completo em `docs/architecture/multi-user-b2b-wave-b2b5-scope.md` e `decisions-log.md` D-095. **Já apresentado ao Marcelo — implementação não deve começar sem o sinal dele.**

## O que está em andamento

1. **Multi-User B2B — autorizado a prosseguir, Waves B2B-0 a B2B-4 `DONE` (D-084 a D-094), Wave B2B-5 com escopo `APPROVED` (D-095).** Design técnico `APPROVED` (protocolo completo em `roadmap-evolution/17-multi-user-b2b-revised-strategy.md` §125) e timing decidido diretamente pelo Marcelo. Estado detalhado por subitem (evidência `DoD:`, judgment calls de sequenciamento documentados) vive em `docs/architecture/multi-user-b2b-wave-tracker.md` — não duplicado aqui. Resumo: **B2B-0** inventário read-only; **B2B-1** physical model formalizado via protocolo Claude↔Codex 5 rodadas (D-086, `docs/architecture/multi-user-b2b-physical-model.md`); **B2B-2** fundação de identidade global aditiva (D-087/D-088) — unifica os 2 caminhos de login no `TenantBootstrapService` já fenced, fechando o gap de fencing do BFF achado na Wave B2B-0; **B2B-3** `Organization`/`Membership`/`CreateOrganizationService` (D-089/D-090/D-091) — isolamento IAM do GSI4, domain/persistence, transação atômica de criação, ainda não wireado a nenhum fluxo de login/onboarding; **B2B-4** `OnboardingStateResolver` (D-092/D-094) — classificador puro de 4 estados, ainda sem wiring de login nem exposição HTTP (por design, ver D-092).

   **Próxima wave real: B2B-5 (RequestContext Cutover, §110 do roadmap) — escopo já `APPROVED` via protocolo (D-095, `docs/architecture/multi-user-b2b-wave-b2b5-scope.md`), já apresentado ao Marcelo. NÃO iniciar a implementação sem o sinal dele primeiro.** É a primeira wave que muda `identity → Organization selection → Membership → permission → lifecycle → RequestContext` de verdade e elimina o fallback `tenantId=userId` — mexe em `resolve-request-context.ts`/`bootstrap-identity.ts`/`bff-auth-service.ts`, o núcleo do login real. Isto não é gate do protocolo Claude↔Codex (já rodado e fechado 9,1/9,1, D-095) — é uma checagem de alinhamento adicional antes de tocar o núcleo do login, per a própria recomendação da sessão que fechou B2B-4. Pendência genuína de confirmação do Marcelo, não bloqueio técnico nem de protocolo.
...
|---|---|---|
| Wave B2B-5 (RequestContext Cutover) — sinal para implementar | Escopo já `APPROVED` via protocolo (D-095) e já apresentado ao Marcelo — falta o sinal dele para começar a codificar (mexe no fluxo de login real, elimina fallback `tenantId=userId`) | `docs/architecture/multi-user-b2b-wave-b2b5-scope.md` |
...
1. **Multi-User B2B, Wave B2B-5 (RequestContext Cutover)** — escopo `APPROVED` (D-095), já apresentado ao Marcelo; aguardando o sinal dele para implementar (item 1 de "O que está em andamento"). Waves B2B-0 a B2B-4 `DONE`.
