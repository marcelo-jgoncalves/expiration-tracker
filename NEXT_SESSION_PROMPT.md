...
8. compartilhamento externo seguro (`ExternalShareLink`) — 🟢 **FECHADO POR COMPLETO (D-273), 2026-09-12** — design `APPROVED` (D-225), slice 1/3 (D-241) + slice 2/3 (rotas HTTP autenticadas+anônima, RBAC, schemas, terraform, BFF, testes) ambas implementadas. 2 achados reais corrigidos durante slice 2/3 (não presentes na slice 1/3 original): pepper de auditoria de IP reusava o pepper do token (violação da Decisão 11, corrigido com `ipAuditPepper` próprio); bug de forma do token no `handleCreateShareLink` (string composta `shareId.selector.secret` do serviço nunca dividida antes de devolver ao cliente — achado só por teste de integração real, não pelos unit tests do serviço). Gate local completo verde (`npm test` 2926/2926), `terraform test` root 27/27 + módulo `api-gateway` 1/1, `terraform plan` contra `dev` confirmou exatamente os recursos esperados (nunca aplicado). **Último item do backlog P1 — backlog P1 inteiro está fechado.** Pendente: merge `develop→main`/`terraform apply` (decisão explícita de quem retomar, ver `docs/architecture/decisions-log.md` D-273).
...
**`ExternalShareLink` slice 2/3 FECHADA (D-273), 2026-09-12 — backlog P1 inteiro está fechado.** Commitado em `develop`, gate local completo verde, terraform validado/testado/planejado contra `dev` (nunca aplicado).

**Próximo passo imediato**: decidir merge `develop→main`/`terraform apply` do trabalho acumulado (D-273 + qualquer pendência anterior ainda não aplicada em `dev`) — varredura coordenada, mesmo padrão já usado para as fatias WhatsApp 4/5+5/5. Depois disso (ou em paralelo, à critério de quem retomar): eixos do full-audit round2 que ainda têm trabalho decisão-independente real — E-021 (dashboard operacional/EMF ausente, fan-out sem cap em `onboarding-state.ts`/`resolve-active-membership.ts`), E-016 (`reset-dev-data.ts`'s `QUEUE_BASE_NAMES` desatualizado), E-017 (gate de evidência ponta-a-ponta para fechar item de ROADMAP, limite de profundidade de subagente). E-015/E-019 dependem mais de decisão/produto que de engenharia pura.
