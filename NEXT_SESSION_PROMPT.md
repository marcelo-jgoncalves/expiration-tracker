...
27. Worker de entrega de e-mail (M4) — 3 achados estruturais reais encontrados durante a revisão adversarial de D-332 (2026-09-24), 🔴 NÃO corrigidos, pendência formal para uma rodada de protocolo dedicada: (1) `DEFERRED`/quiet hours nunca é realmente reagendado - perda silenciosa e permanente da notificação (comentário do código que afirmava um reagendamento via EventBridge Scheduler foi corrigido para não mentir, mas o mecanismo real nunca foi implementado); (2) `FAILED_RETRYABLE` (ex. throttling do SES) é confirmado como sucesso pelo handler SQS, sem reconciliador; (3) falta de revalidação de preference/entitlement/policy entre o roteamento e o envio real (uma revogação de consentimento no meio do caminho não é respeitada). Detalhe: `decisions-log.md` D-332, `docs/architecture/reviews/d315-d316-notification-entitlements-urgency-adversarial-review/round-1-codex-output.txt`.
...
29. Retirada do "Rastreamento legado" (A10) + entrega de solicitação por A14 — **APROVADO via protocolo + decisão de produto de Marcelo (D-334, 2026-09-25)**: descartar vínculo `ExpirationItem` sem substituto; retirada completa; abandonar as 3 capacidades órfãs. 🔴 Implementação ainda NÃO iniciada — grande (backend módulo `subject` inteiro + infra G01 + frontend + migração de A22 para `document-archive`), sequenciada depois do item 32 (correção dos 28 e2e) para não colidir em arquivos. Detalhe: `decisions-log.md` D-334, `docs/architecture/adr/ADR-0016-legacy-tracking-retirement.md`.
32. 28 testes Playwright falharam no CI após o push do redesign OmniVence (run `36148438004`, job `frontend`) — maioria são specs antigas nunca revalidadas contra a UI nova (ex. texto de empty state mudou). Agente em background corrigindo (triagem expectativa-desatualizada vs. bug real), resultado ainda não reportado nesta sessão.
30. Auditoria de arquitetura de dados (modelo single-table DynamoDB, GSIs, chaves, exceções de particionamento GSI3/GSI6) — sugerida por Marcelo (2026-09-25), pergunta em aberto se vale a pena, não iniciada. A retirada do item 29 já achou um sintoma concreto (dois modelos de conformidade paralelos, M9 vs M10+, coexistindo sem que nenhuma auditoria tivesse sinalizado antes) que é evidência a favor, mas a decisão de fazer ou não a auditoria continua com Marcelo.
31. Análise de telas existentes que poderiam virar modais em vez de rota própria — pedido por Marcelo (2026-09-25), motivado pela sensação de telas pequenas/contextuais demais (ex. hub do fornecedor) durante o port de `prototype/novasTelas`. Não iniciada ainda. Relacionada à preferência já registrada de usar `Dialog` para interações curtas e contextuais em vez de nova rota (ver memória do agente); esta é a varredura do estoque atual de telas contra esse critério.
...
**Ordem de prioridade do protocolo Claude↔Codex** (retomado 2026-09-23/24, ver itens acima): ~~item 22~~ (D-330) → ~~item 12~~ (D-331) → ~~item 20~~ (D-332) → item 26 (WhatsApp phone confirmation) → item 27 (worker de e-mail M4, achados estruturais de D-332) → item 11 (reminder default local time, voluntário). Item 19 suspenso, não entra na fila.

**Última sessão (2026-09-23/24)**: D-329 (login/D-321), D-330 (bulk import/D-319), D-331 (ADR-0014/separação de ambientes) e D-332 (NotificationEntitlements/preferences/urgência) — as 4 revisões adversariais concluídas e aprovadas, ver itens 23/22/12/20 acima. D-332 abriu um item novo (27) para 3 achados estruturais do worker de e-mail M4, não corrigidos por proporcionalidade de escopo. D-328 (WhatsApp phone confirmation) implementado, ainda na fila do protocolo (item 26, próximo). Este arquivo recompactado (guardrail de `check-doc-drift.ts` estourado, ver nota do cabeçalho).
