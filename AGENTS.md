...
**Sem usuários reais nem produção — não pesar "risco ao usuário" (2026-08-30)**: fase de construção do produto, sem usuário real, sem ambiente de produção (deploy real só existe em `dev`, dado sintético, resetável/reseedável a qualquer momento), sem data de lançamento nem pressão de prazo. Objetivo é qualidade máxima em ritmo profissional decente, não velocidade. Por isso, ao avaliar uma mudança, **nunca tratar "quebraria contas/sessões/dados existentes em `dev`" como risco bloqueador por si só** — essa categoria de cautela (compatibilidade retroativa, migração seguindo usuário real) não se aplica aqui e não deve aparecer como justificativa em análise própria ou rodada Claude↔Codex. Isto **não** dispensa rigor de engenharia, segurança, ou o protocolo do §4 para decisões Type 1 — só remove a categoria específica de risco de "impacto a usuário/produção" que este projeto ainda não tem. Um bloqueador real continua sendo: qualidade de engenharia insuficiente, achado de segurança, ou perda de produtividade do próprio time (ex. quebrar o ambiente de trabalho corrente sem necessidade).
...
**Pesquisa externa antes da Rodada 1 (2026-08-30, E-014)**: uma decisão nível 5-6 que define um padrão que sistemas fora deste projeto já resolveram de forma estabelecida (RBAC, convite/invite, sessão multi-tenant, etc.) exige, na própria proposta Rodada 1, uma declaração explícita `SIM`/`SIM PARCIAL`/`NÃO` (nunca omissão) e, se `SIM`/`SIM PARCIAL`, um checklist de critérios pesados derivado da pesquisa que vira a régua de nota da rodada — critério completo, fluxo de reconciliação se o Codex contestar o checklist, e exemplos: `docs/engineering/research-protocol.md`.
...
**Infra (ADR-0009): CDK substituído por Terraform**, `infra/` (módulos + wiring raiz), deploy real via GitHub Actions (`ci.yml` plan-only em PR, `cd.yml` apply em push a `main`, OIDC sem credencial de longa duração). Regra de isolamento de índice: nunca conceder acesso de leitura a um GSI restrito (GSI3/GSI6) via política geral de tabela — só via política escopada explicitamente ao índice (`infra/modules/dynamo-table/main.tf`), anexada só às roles que precisam. Suíte de infra (`infra/tests/stack.tftest.hcl`, `terraform test`) prova isolamento de GSI3/GSI6, DLQ, alarmes, contrato do EventBridge Scheduler. **Acesso AWS real (verificação/smoke test, nunca `apply` local) é via `aws ... --profile claude-dev`** (conta `dev`) — nunca concluir "sem acesso AWS" a partir só do CLI sem profile (retorna `NoCredentials` mesmo quando `claude-dev` funciona). **`terraform force-unlock` no backend `dev` está pré-autorizado sem confirmação prévia (Marcelo, 2026-08-31)** — lock travado é artefato conhecido de múltiplas sessões/agentes rodando `terraform plan`/`test`/`apply` neste mesmo backend; `dev` não tem usuário real, o risco de destravar incorretamente é aceitável aqui (checar `gh run list` antes, para confirmar que nenhum workflow real está em andamento, é boa prática mas não bloqueia a ação).
