...
M0 ("guardrails e contratos", `implementation-blueprint.md` §19) está implementado: `src/shared/{errors,observability,config,idempotency,dynamodb,outbox,contracts}`, schemas em `schemas/`, testes em `test/{unit,contract}`. M1 adicionou `src/modules/identity/{domain,application,ports,persistence,http}` (resolver central, matriz de autorização, quotas). M2 adicionou `src/modules/expiration/{domain,application,ports,http}` (port `ExpirationStore` introduz `transactWrite`, reusado por M3's `ReminderStore`). M3 adicionou `src/modules/reminder/{domain,application,ports,http}` + `src/workers/{reminder-producer,reminder-dispatch,reminder-reconciliation}` (lógica pura, testável com relógio injetado). M3.5 adicionou runtime Lambda real em `src/runtime/aws/handlers/` — a lógica pura em `src/workers/` continua observability-agnostic por decisão (`decisions-log.md` E-007). M4+ segue o mesmo layout (`src/modules/<módulo>/...`, `src/workers/<worker>`) descrito em `implementation-blueprint.md` §2. M4 (design `APPROVED`, `docs/architecture/m4-notification-engine-design.md`) tem a Camada 1 implementada — `src/modules/notification/{domain,ports,application}` (router com matriz fail-closed/fail-open, quiet hours DST-aware reusando `reminder/domain/recurrence.ts`, REPLACEMENT/CORRECTIVE, estados de ambiguidade do SES, transições monotônicas do callback) — mesmo padrão de M3: lógica pura primeiro, adapters DynamoDB reais/wiring Terraform/handlers Lambda ficam para uma fase de runtime real subsequente, não fechada ainda.

**Infra (ADR-0009, 2026-08-20): CDK substituído por Terraform**, `infra-terraform/` (módulos + wiring raiz), deploy real via GitHub Actions (`ci.yml` plan-only em PR, `cd.yml` apply em push a `main`, OIDC sem credencial de longa duração). Regra de isolamento de índice descoberta em M3, ainda válida: nunca conceder acesso de leitura a um GSI restrito (GSI3/GSI6) via política geral de tabela — só via política escopada explicitamente ao índice (`infra-terraform/modules/dynamo-table/main.tf`), anexada só às roles que precisam. Suíte de infra (`infra-terraform/tests/stack.tftest.hcl`, `terraform test`) recria as propriedades que a antiga suíte CDK provava — isolamento de GSI3/GSI6, DLQ, alarmes, contrato do EventBridge Scheduler.
