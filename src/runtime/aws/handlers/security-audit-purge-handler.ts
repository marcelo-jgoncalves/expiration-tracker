...
import { DynamoDbSecurityAuditPurgeCandidateSource } from "../../../workers/security-audit-purge/dynamodb-candidate-source.js";
...
async function handlePurge(event: SecurityAuditPurgeEvent): Promise<void> {
  const result = await runSecurityAuditPurge({ candidates, tableName: tableName as string, now: () => new Date().toISOString() });
