...
const { store, candidateSource, dueWorkTableName, dispatchOutboxTableName, now, newEventId, correlationId } = buildReconciliationDeps(client, tableName);
...
async function handleReconciliation(event: ReminderReconciliationEvent): Promise<void> {
  ...
  const claimsRecovered = pagedRecovery
    ? await recoverExpiredClaims({ store, tableName, dueWorkTableName, dispatchOutboxTableName, now, newEventId, correlationId, claimTtlMs: 120_000 }, expiredClaimCandidates)
    : 0;
