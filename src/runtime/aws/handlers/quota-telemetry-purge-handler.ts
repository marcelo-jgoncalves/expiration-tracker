...
import { DynamoDbQuotaTelemetryPurgeCandidateSource } from "../../../workers/quota-telemetry-purge/dynamodb-candidate-source.js";
...
async function handlePurge(event: QuotaTelemetryPurgeEvent): Promise<void> {
  const result = await runQuotaTelemetryPurge({ candidates, tableName: tableName as string, now: () => new Date().toISOString() });
