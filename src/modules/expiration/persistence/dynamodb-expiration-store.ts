...
export class DynamoDbExpirationStore implements ExpirationStore {
  ...
  async transactWrite(entries: TransactWriteEntry[]): Promise<void> {
    try {
      ...
      // TransactionCanceledException must reach the caller as-is (isTransactionCanceled()
      // is how callers distinguish OCC/idempotency conflicts from other failures) - never
      // wrapped by mapDynamoError, which would hide that name.
      throw err;
    }
  }
