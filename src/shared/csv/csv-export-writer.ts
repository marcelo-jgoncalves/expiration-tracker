 ...
 */
import { sanitizeFormulaInjection } from "../spreadsheet/formula-injection.js";

/**
 * Serializes one CSV field: prefixes a leading apostrophe when the raw value starts with a
 * formula-trigger character (so a spreadsheet reads it as literal text, not a formula - see
 * `sanitizeFormulaInjection`), then applies RFC4180 quoting (wrap in double quotes when the
 * value contains a comma, double quote, or newline; double any internal double quote) to the
 * RESULT of that prefixing step. Order matters: mitigating first means the mitigation's own
 * `'` character is what a downstream quoting pass sees, not a value that already has quotes
 * needing separate escaping.
 */
export function serializeCsvField(value: string): string {
  const mitigated = sanitizeFormulaInjection(value);
